...
class ContextEngineer:
    ...
    def process_document_with_context(self, content: str, metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"Documento procesado: {len(chunks)} chunks con contexto cultural")
            return chunks
    ...
    def _enrich_chunk_with_cultural_context(self, chunk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _identify_community_resources(self, content: str) -> List[str]:
        ...
        resource_patterns = {
            'autoridad_local': ['gobernador', 'teniente gobernador', 'alcalde', 'varayoc'],
            'servicio_social': ['puesto de salud', 'centro de salud', 'comedor popular'],
    ...
    def _identify_access_barriers(self, content: str) -> List[str]:
        ...
        barrier_indicators = {
            ...
            'educativas': ['no sé leer', 'no sé escribir', 'analfabeto', 'sin estudios']
        }
    ...
    def _detect_legal_type(self, content: str) -> str:
        ...
        type_patterns = {
            'violencia_familiar': ['violencia', 'maltrato', 'agresión', 'protección', 'medida'],
    ...
    def _extract_legal_entities(self, content: str) -> List[str]:
        ...
        entity_patterns = {
            'instituciones': ['juzgado', 'comisaría', 'mimp', 'reniec', 'municipalidad'],
    ...
    def _identify_procedures(self, content: str) -> List[str]:
        ...
        procedure_keywords = [
            'denunciar', 'demandar', 'solicitar', 'presentar', 'inscribir',
            'tramitar', 'pedir', 'reclamar', 'apelar', 'interponer'
        ]
    ...
    def _assess_urgency(self, content: str) -> str:
        ...
        high_urgency = ['urgente', 'inmediato', 'emergencia', 'peligro', 'violencia', 'agresión']
    ...
    def get_contextualized_prompt(self, query: str, agent_type: str, user_context: Optional[Dict[str, Any]] = None) -> Tuple[str, Dict[str, Any]]:
        """
    ...
    def get_cultural_recommendations(self, context: Dict[str, Any]) -> List[str]:
        ...
        if location and location in self.cultural_db['communities']:
            ...
            recommendations.append(f"Considerar contexto cultural: {location_info.get('cultural_notes', '')}")
    ...
    def validate_response_cultural_appropriateness(self, response: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if context.get('cultural_context', {}).get('requires_cultural_adaptation'):
            ...
            if not any(term in response.lower() for term in cultural_terms):
                ...
                validation_result['recommendations'].append("Incluir términos culturales")
        ...
        if context.get('query_language') == 'quechua':
            if not self._has_quechua_content(response):
                ...
                validation_result['recommendations'].append("Agregar traducción al quechua")
        ...
        validation_result['cultural_score'] = max(0.0, validation_result['cultural_score'] + 0.7)
        
        return validation_result
    
    def _has_quechua_content(self, text: str) -> bool:
        """Verifica si el texto tiene contenido en quechua"""
        quechua_indicators = ['ñuqa', 'qam', 'pay', 'ñuqanchik', 'qamkuna', 'paykuna', 'kani', 'kanki', 'kanku']
    ...
    def _is_too_complex(self, text: str) -> bool:
        ...
        complex_words = ['constitucional', 'jurisdicción', 'competencia', 'imperativo', 'inherente']
