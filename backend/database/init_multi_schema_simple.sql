-- Esquema multi-schema para IA Jurídica - Versión Simplificada (sin vector)
-- PostgreSQL + Redis Integration

-- Extensiones necesarias
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pg_trgm";
-- CREATE EXTENSION IF NOT EXISTS "vector"; -- Comentado por ahora
...
CREATE TABLE IF NOT EXISTS conversations_schema.messages (
    ...
    language VARCHAR(10) DEFAULT 'spanish',
    metadata JSONB DEFAULT '{}',
    -- embedding vector(1024), -- Comentado por ahora
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- CREATE INDEX IF NOT EXISTS idx_messages_embedding ON conversations_schema.messages USING ivfflat (embedding vector_cosine_ops); -- Comentado
...
-- CREATE INDEX IF NOT EXISTS idx_chunks_embedding ON rag_schema.document_chunks USING ivfflat (embedding vector_cosine_ops); -- Comentado
